(function() {
  ...
  angular
    ...
    .service("FriendsList", FriendsList)
    ...
    FriendsList.$inject = ["$firebaseArray", "CONFIG", "UserService"];
    ...
    function FriendsList($firebaseArray, CONFIG, UserService){
      var ref = new Firebase(CONFIG.FIREBASE_URL);
      var currentUser = UserService.getProfile();
      var friends = $firebaseArray(ref.child('users'));
      return {
        all: function () {
          friends.$loaded().then(function(response){
            angular.forEach(response, function(friend){
              console.log(friend['username']);
            })
          });
          friends.$remove(currentUser.id);
          return friends;
        },

        get: function (userId) {
          return friends.$getRecord(userId);
        },
